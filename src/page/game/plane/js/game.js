...
import { clearObstruction, initObstruction, mapArr, obstructs } from './obstruction/obstruction.js'
import { findRoute } from './findRoute/findRoute.js'
import { fullScreenBombing, hellfire, skills, vineCage } from './skill/skill.js'
/*----------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------*/
...
export function gameLoad() {
    ...
    function characterActions() {
        ...
        characterShoot(function (flag) {
            // flag = 1 普通攻击即(非技能攻击) flag > 1 技能攻击
            if (flag === 1) {
    ...
    function collideActions() {
        ...
        // 障碍物和角色子弹
        collide1(obstructs, bulletArr, function (result) {
            // 去除碰撞后角色子弹
            // 数组去重
            const s2 = [...new Set(result[1])];
            // 降序目的是为了下面使用splice的时候不用考虑数组变化了导致下标对不上
        ...
        // 障碍物和怪物子弹
        collide1(obstructs, bulletMArr, function (result) {
            // 去除碰撞后的怪物子弹
            // 数组去重
            const s2 = [...new Set(result[1])];
            // 降序目的是为了下面使用splice的时候不用考虑数组变化了导致下标对不上
    ...
    function monsterActions() {
        ...
        monsterMove(character, 1, findRoute, function (monster) {
            // 这个函数在一次移动完后调用
            // 攻击
            monsterShoot(monster, function (flag) {
                // 使用技能
                if (flag === 1) {
                    // 发射子弹
                    addMBullet(monster);
                    ...
                    // 藤蔓囚笼
                    vineCage(monster,character,mapArr);
                    monster.shootTimer = 9999;
                }
            });
        });
    }
